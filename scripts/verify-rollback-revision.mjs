...
function expectedPlainEnvironment(worker, { rollbackSha, rollbackParentSha, environment, bucket }) {
  const expected = {
    GCS_BUCKET_NAME: bucket,
    URAI_ENV: environment,
    URAI_ROLLBACK_SHA: rollbackParentSha,
...
export function inspectRollbackRevision({
  ...
  if (!nonEmptyString(rollbackRevision) || !rollbackRevision.startsWith(`${worker}-`)) failures.push(`rollback revision ${rollbackRevision || '<missing>'} does not belong to ${worker}`);
  ...
  const rollbackParentSha = String(plainEnv.URAI_ROLLBACK_SHA ?? '');
  const expectedPlainEnv = expectedPlainEnvironment(worker, {
    rollbackSha,
    rollbackParentSha,
    environment,
    bucket,
  });
  const expectedSecrets = requiredSecretNames(worker);
  ...
  if (!SHA_PATTERN.test(rollbackParentSha)) failures.push('rollback revision URAI_ROLLBACK_SHA must be a full lowercase 40-character SHA');
  if (rollbackParentSha === rollbackSha) failures.push('rollback revision URAI_ROLLBACK_SHA must differ from URAI_SOURCE_SHA');

  if (JSON.stringify(Object.keys(plainEnv).sort()) !== JSON.stringify(Object.keys(expectedPlainEnv).sort())) {
    failures.push(`rollback revision plain environment keys ${JSON.stringify(Object.keys(plainEnv).sort())} do not match approved keys ${JSON.stringify(Object.keys(expectedPlainEnv).sort())}`);
  }
  ...
  if (JSON.stringify(Object.keys(secretVersions).sort()) !== JSON.stringify([...expectedSecrets].sort())) {
    failures.push(`rollback revision secret bindings ${JSON.stringify(Object.keys(secretVersions).sort())} do not match approved bindings ${JSON.stringify([...expectedSecrets].sort())}`);
  }
  for (const name of expectedSecrets) {
    if (!NUMERIC_SECRET_VERSION_PATTERN.test(String(secretVersions[name] ?? ''))) failures.push(`rollback revision ${name} must use an exact numeric Secret Manager version`);
  }
  ...
  if (configFingerprint !== approvedConfigFingerprint) failures.push(`rollback revision configuration fingerprint ${configFingerprint} does not match explicitly approved fingerprint ${approvedConfigFingerprint}`);

  if (failures.length > 0) throw new Error(`[FAIL] ${worker} rollback configuration is not release-authorized:\n- ${failures.join('\n- ')}`);
  return {
    worker,
    rollbackRevision,
    imageDigest,
...
export function validateRollbackConfigurationReceipt(receipt) {
  const failures = [];
  if (!receipt || typeof receipt !== 'object' || Array.isArray(receipt)) throw new Error('rollback configuration receipt must be a JSON object');
  ...
  if (!Array.isArray(receipt.services) || receipt.services.length === 0) failures.push('services must contain at least one rollback revision');
  else {
    const seen = new Set();
    for (const [index, service] of receipt.services.entries()) {
      ...
      const rollbackParentSha = String(service?.plainEnv?.URAI_ROLLBACK_SHA ?? '');
      if (!SHA_PATTERN.test(rollbackParentSha)) failures.push(`${prefix}.plainEnv.URAI_ROLLBACK_SHA must be a full lowercase 40-character SHA`);
      if (rollbackParentSha === receipt.rollbackSha) failures.push(`${prefix}.plainEnv.URAI_ROLLBACK_SHA must differ from rollbackSha`);
      ...
      if (!service.secretVersions || typeof service.secretVersions !== 'object' || Array.isArray(service.secretVersions)) failures.push(`${prefix}.secretVersions must be an object`);
      else {
      ...
      const expectedConfiguration = canonicalConfiguration({
...
function buildValidReceipt() {
  ...
  const rollbackParentSha = 'd'.repeat(40);
  ...
  const revisionLabels = { 'urai-source-sha': rollbackSha, 'urai-environment': environment };
  const plainEnv = expectedPlainEnvironment(worker, { rollbackSha, rollbackParentSha, environment, bucket: artifactBucket });
  ...
  return {
    ...
    services: [{
      ...
      configuration,
      configFingerprint,
      approvedConfigFingerprint: configFingerprint,
      status: 'PASS',
    }],
  };
}
...
export function runRollbackRevisionVerifierSelfTest() {
  ...
  expectRejected('rollback ancestor mismatch', (fixture) => { fixture.services[0].plainEnv.URAI_ROLLBACK_SHA = fixture.rollbackSha; });
