...
write_functions_env() {
  ...
  for key in NARRATOR_WORKER_URL ASSET_WORKER_URL SPATIAL_WORKER_URL STUDIO_WORKER_URL WEBHOOK_SIGNING_SECRET GCS_BUCKET_NAME API_ALLOWED_ORIGINS URAI_ENV GCP_REGION GCLOUD_PROJECT GOOGLE_CLOUD_PROJECT FIREBASE_PROJECT_ID; do
    if [ -z "${!key:-}" ]; then
...
SPATIAL_WORKER_URL=$SPATIAL_WORKER_URL
STUDIO_WORKER_URL=$STUDIO_WORKER_URL
EOF

  echo "[PASS] Firebase Functions runtime env file prepared."
}
