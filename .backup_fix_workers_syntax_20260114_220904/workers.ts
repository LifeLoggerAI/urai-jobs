...
export type WorkerResult = { ok: true; result?: any } | { ok: false; error: any };
...
async function doWebhook(payload: any): Promise<WorkerResult> {
  ...
  try {
    ...
    clearTimeout(to);
      return (r.ok
        ? { ok: true, result: { status: r.status, body: txt.slice(0, 20000) } }
        : { ok: false, error: { code: "http_error", status: r.status, body: txt.slice(0, 20000) } });
